...
exports.getFournisseursParPays = async (req, res) => {
  try {
    const fournisseurs = await Fournisseur.aggregate([
      {
        $group: {
          _id: "$pays", // Groupement par pays
          count: { $sum: 1 }, // Compte des fournisseurs par pays
        },
      },
      {
        $sort: { _id: 1 }, // Tri des résultats par pays
      },
    ]);

    res.status(200).json(fournisseurs); // Renvoie les résultats
  } catch (err) {
    console.error('Erreur dans getFournisseursParPays:', err);
